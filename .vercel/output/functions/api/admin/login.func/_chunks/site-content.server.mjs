...
async function createPgliteSql() {
	...
	const migrate = async () => {
		const migrations = /* #__PURE__ */ Object.assign({ "/migrations/0001_site_content.sql": _0001_site_content_default });
...
//#endregion
//#region src/lib/site-content.server.ts
...
async function readSiteContent() {
	const rows = await (await getSql())`select content from site_content where id = true`;
	return rows[0] ? cleanContent(rows[0].content) : defaultSiteContent;
}
async function updateSiteContent(change) {
...
async function createSiteVersion(label) {
	const sql = await getSql();
...
async function listSiteVersions() {
	return (await (await getSql())`select id, label, content, created_at from site_versions order by created_at desc limit 5`).map((row) => ({
...
async function restoreSiteVersion(id) {
	const rows = await (await getSql())`select content from site_versions where id = ${id}`;
	if (!rows[0]) throw new Error("Version not found");
	return updateSiteContent(rows[0].content);
}
//#endregion
export { updateSiteContent as a, restoreSiteVersion as i, listSiteVersions as n, readSiteContent as r, createSiteVersion as t };
